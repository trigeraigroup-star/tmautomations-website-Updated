...
export async function deliverContact(rawValues,env,fetchImpl=fetch){
 ...
 if(!env.RESEND_API_KEY) throw new ContactError(503,'Email delivery is not connected yet. Your details have not been sent. Please call (850) 775-6909.');
 ...
 if(!res.ok){const detail=typeof res.text==='function'?await res.text().catch(()=>''):'';console.error('[contact] Resend rejected the email',res.status,detail.slice(0,300));throw new ContactError(502,'We could not send your message right now. Please try again or call (850) 775-6909.');}
 return {ok:true};
}
